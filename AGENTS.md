...
- Public Easter card `/yefasika-beal` uses `PublicYefasikaBealController`, `resources/views/public/yefasika-beal.blade.php`, strings in `lang/*/app.php` under the Yefasika Beal section, and `resources/views/public/partials/yefasika-beal-background.blade.php` for fixed full-viewport particle canvases (back layer behind content, front layer above cards; both non-interactive).
