{{-- Fasika Quiz: teaser card on page + full modal for the quiz itself --}}

{{-- ══════════════════════════════════════════════════════
     TEASER CARD (always visible on page)
══════════════════════════════════════════════════════ --}}
<section class="relative mx-auto w-full max-w-md rounded-2xl border border-[#e2ca18]/[0.22] bg-gradient-to-br from-amber-950/55 via-[#1a1210]/78 to-[#0a2832]/72 px-5 py-6 shadow-[0_16px_36px_-12px_rgba(30,16,8,0.42)] ring-1 ring-inset ring-[#f5d060]/[0.06] backdrop-blur-[3px] backdrop-saturate-125 sm:max-w-lg sm:px-7 sm:py-7"
    ...
    <template x-teleport="body">
        <div x-show="modalOpen"
             x-cloak
             class="fixed inset-0 z-[9000] flex items-end justify-center sm:items-center sm:p-4"
            ...
            {{-- Modal card --}}
            <div class="relative z-10 w-full max-w-md rounded-t-3xl bg-[#0f0a1a] shadow-2xl ring-1 ring-white/10 sm:rounded-3xl sm:max-w-lg flex flex-col"
                ...
                {{-- Drag handle (mobile) --}}
                <div class="mx-auto mt-3 h-1 w-10 rounded-full bg-white/20 sm:hidden shrink-0"></div>

                {{-- ── STATE: name + start ── --}}
                <div x-show="state === 'intro'" class="flex flex-col overflow-y-auto">
                    <div class="flex items-center justify-between px-5 pt-5 pb-4 border-b border-white/[0.07] shrink-0 sm:px-7">
                    ...
                    <div class="px-5 py-6 space-y-5 sm:px-7">
                        <p class="text-sm leading-relaxed text-zinc-300/80">
                            15 ጥያቄዎች · 10 ደቂቃ · ከቀላል እስከ ከባድ
                        </p>

                        <div>
                            <label for="fq-name" class="block text-xs font-semibold text-zinc-400 mb-2">ስምዎ (አማራጭ — ለሰንጠረዥ ይጠቅማል)</label>
                            <input id="fq-name"
                ...
                {{-- ── STATE: playing ── --}}
                <div x-show="state === 'playing'" x-cloak class="flex flex-col min-h-0">

                    {{-- Header: timer + progress --}}
                    <div class="flex items-center gap-3 px-5 pt-4 pb-3 border-b border-white/[0.07] shrink-0 sm:px-7">
                        {{-- Timer --}}
                    ...
                    <div class="overflow-y-auto flex-1 px-5 py-5 sm:px-7 space-y-4">
                        ...
                        <div class="flex gap-3 pt-2">
                            <button type="button"
                                ...
                                <svg class="h-4 w-4 shrink-0 rtl:rotate-180" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
function fasikaQuiz(config) {
    return {
        ...
        answeredMap: {},  // { questionId: { selectedOption, feedback } }

        get currentQuestion() { return this.questions[this.currentIndex] ?? null; },
        get hasAnsweredCurrent() {
            return this.currentQuestion && !!this.answeredMap[this.currentQuestion.id];
        },
        get progress() {
            return this.questions.length ? Math.round((this.currentIndex / this.questions.length) * 100) : 0;
        },
        get formattedTime() {
        ...
        maybeClose() {
            if (this.state === 'playing') return; // don't close mid-quiz on backdrop click
            this.closeModal();
        },
        ...
        async selectOption(opt) {
            ...
            try {
                ...
                const fallback = { is_correct: false, correct_option: opt, points_earned: 0 };
                this.feedback = fallback;
                this.answeredMap[qId] = { selectedOption: opt, feedback: fallback };
        ...
        restoreAnswerState() {
            ...
            if (!q) return;
            const saved = this.answeredMap[q.id];
            if (saved) {
        ...
        async finishQuiz() {
            ...
            try {
                const res = await fetch(config.completeUrl, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json', 'Accept': 'application/json', 'X-CSRF-TOKEN': config.csrf },
                    body: JSON.stringify({ token: this.token, name: this.participantName || null, answers: this.answers, time_taken_seconds: timeTaken }),
                });
                this.results = await res.json();
            } catch {
                this.results = { score: this.score, total_possible: 30, percentage: Math.round((this.score / 30) * 100), correct_count: this.answers.filter(a => a.is_correct).length, total_questions: this.answers.length, leaderboard: [] };
            }
            this.state = 'results';
        },
        ...
        resetQuiz() {
            ...
            this.selectedOption = null; this.feedback = null; this.answers = []; this.answeredMap = {}; this.score = 0;
            this.timeLeft = 600; this.results = null; this.errorMessage = '';
        },
    };
}
</script>
