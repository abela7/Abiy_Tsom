            ...
            <div class="flex items-center gap-0.5 shrink-0">
                @if($authUser)
                    <a href="{{ route('admin.suggestions.my') }}" class="p-2 rounded-xl hover:bg-muted transition touch-manipulation" title="{{ __('app.suggest_my_suggestions') }}">
                        <svg class="w-5 h-5 text-muted-text" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/></svg>
                    </a>
                @endif
        ...
        <div class="max-w-lg mx-auto px-4 py-5 pb-8">
            ...
            <div x-show="step === 1" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-x-4" x-transition:enter-end="opacity-100 translate-x-0">
                ...
                <div class="grid grid-cols-2 gap-3">
                    <button type="button" @click="language = 'en'; goStep(2)"
                            class="flex flex-col items-center gap-3 p-6 rounded-2xl border-2 transition touch-manipulation active:scale-[0.96]"
                            :class="language === 'en' ? 'border-accent bg-accent/5' : 'border-border bg-card hover:border-accent/40'">
                        <span class="text-3xl">🇬🇧</span>
                        <span class="text-sm font-bold text-primary">English</span>
                    </button>
                    <button type="button" @click="language = 'am'; goStep(2)"
                            class="flex flex-col items-center gap-3 p-6 rounded-2xl border-2 transition touch-manipulation active:scale-[0.96]"
                            :class="language === 'am' ? 'border-accent bg-accent/5' : 'border-border bg-card hover:border-accent/40'">
                        <span class="text-3xl">🇪🇹</span>
            ...
            {{-- ════════════════════════════════════════════════════════════════ --}}
            {{-- STEP 2 : What do you want to suggest?                           --}}
            {{-- ════════════════════════════════════════════════════════════════ --}}
            <div x-show="step === 2" x-cloak x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-x-4" x-transition:enter-end="opacity-100 translate-x-0">
                ...
                <div class="grid grid-cols-2 gap-2.5">
                    <template x-for="(t, idx) in types" :key="t.value">
                        <button type="button" @click="pickType(t.value)"
                                :class="[
                ...
                <button type="button" @click="goStep(1)" class="w-full mt-5 py-2.5 text-sm font-medium text-muted-text hover:text-primary transition touch-manipulation text-center">
                    &larr; {{ __('app.back') }}
                </button>
            </div>
            ...
            <div x-show="step === 3" x-cloak x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-x-4" x-transition:enter-end="opacity-100 translate-x-0">
                ...
                <form method="POST" action="{{ route('suggest.store') }}" @submit="saveToHistory(); submitting = true">
                    @csrf
                    <input type="hidden" name="language" :value="language">
                    <input type="hidden" name="items[0][type]" :value="currentType">
                    <input type="hidden" name="submitter_name" :value="name">

                    {{-- Type badge + back --}}
                    <div class="flex items-center justify-between mb-4">
                        <button type="button" @click="goStep(2)" class="flex items-center gap-1.5 text-sm font-medium text-muted-text hover:text-primary transition touch-manipulation">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
                            {{ __('app.back') }}
                        </button>
                        <div class="flex items-center gap-2">
                            <span class="px-2.5 py-1 rounded-lg bg-accent/10 text-accent text-xs font-bold" x-text="currentTypeLabel"></span>
                            <span class="text-xs font-bold text-muted-text uppercase" x-text="language"></span>
                        </div>
                    </div>
                    ...
                    <div class="bg-card rounded-2xl border border-border shadow-sm overflow-hidden">
                        <div class="p-4 sm:p-5 space-y-4">
                            ...
                            <div>
                                ...
                                <input type="text" name="items[0][title]" x-model="title" required autofocus
                            ...
                            {{-- Bible: verse range --}}
                            <div x-show="currentType === 'bible'">
                                <label class="block text-xs font-bold text-muted-text uppercase tracking-widest mb-1.5">
                                    {{ __('app.suggest_reference_label') }}
                                </label>
                                <input type="text" :name="currentType === 'bible' ? 'items[0][reference]' : ''" x-model="reference"
                                       placeholder="{{ __('app.suggest_reference_ph') }}"
                                       maxlength="500"
                                       :disabled="currentType !== 'bible'"
                                       class="w-full h-12 px-4 rounded-xl border border-border bg-surface text-primary text-base placeholder:text-muted-text/50 focus:outline-none focus:ring-2 focus:ring-accent/40 focus:border-accent transition">
                            </div>

                            {{-- Mezmur / Book: author --}}
                            <div x-show="currentType === 'mezmur' || currentType === 'book'">
                                <label class="block text-xs font-bold text-muted-text uppercase tracking-widest mb-1.5">
                                    {{ __('app.suggest_author_label') }}
                                </label>
                                <input type="text" name="items[0][author]" x-model="author"
                                       placeholder="{{ __('app.suggest_author_ph') }}"
                                       maxlength="255"
                                       :disabled="currentType !== 'mezmur' && currentType !== 'book'"
                                       class="w-full h-12 px-4 rounded-xl border border-border bg-surface text-primary text-base placeholder:text-muted-text/50 focus:outline-none focus:ring-2 focus:ring-accent/40 focus:border-accent transition">
                            </div>

                            {{-- URL / Link (mezmur, book, reference) --}}
                            <div x-show="currentType === 'mezmur' || currentType === 'book' || currentType === 'reference'">
                                <label class="block text-xs font-bold text-muted-text uppercase tracking-widest mb-1.5">
                                    {{ __('app.suggest_url_label') }}
                                </label>
                                <input type="text" x-model="reference"
                                       :name="currentType !== 'bible' ? 'items[0][reference]' : ''"
                                       placeholder="{{ __('app.suggest_url_ph') }}"
                                       maxlength="500"
                                       :disabled="currentType === 'bible' || currentType === 'sinksar'"
                                       class="w-full h-12 px-4 rounded-xl border border-border bg-surface text-primary text-base placeholder:text-muted-text/50 focus:outline-none focus:ring-2 focus:ring-accent/40 focus:border-accent transition">
                            </div>

                            {{-- Description (all types — optional) --}}
                            <div>
                                <label class="block text-xs font-bold text-muted-text uppercase tracking-widest mb-1.5">
                                    {{ __('app.suggest_detail_label') }}
                                </label>
                                <textarea name="items[0][content_detail]" x-model="detail" rows="3" maxlength="5000"
                            ...
                            {{-- Your name (collapsed by default if not logged in) --}}
                            @if(!$authUser)
                                <div x-data="{ showName: false }">
                                    <button type="button" @click="showName = !showName"
                                            class="text-xs font-medium text-accent hover:underline transition touch-manipulation"
                                            x-text="showName ? '{{ __('app.suggest_submitter_label') }}' : '+ {{ __('app.suggest_submitter_label') }}'">
                                    </button>
                                    <div x-show="showName" x-transition x-cloak class="mt-1.5">
                            ...
                            <button type="submit" :disabled="!title.trim() || submitting"
                                    :class="title.trim() && !submitting ? 'bg-accent text-on-accent hover:bg-accent-hover active:scale-[0.97]' : 'bg-muted text-muted-text cursor-not-allowed'"
                    ...
                    {{-- Suggest another type link --}}
                    <button type="button" @click="goStep(2)"
                            class="w-full mt-3 py-2.5 text-sm font-medium text-muted-text hover:text-accent transition touch-manipulation text-center flex items-center justify-center gap-1.5">
            ...
            <div x-show="step === 4" x-cloak
                ...
                <div class="mt-8 text-center space-y-5">
                    ...
                    <p class="text-sm text-muted-text leading-relaxed max-w-xs mx-auto">
                        {{ __('app.suggest_success_body') }}
                    </p>
                    <div class="flex flex-col gap-2.5 max-w-xs mx-auto">
                        <button type="button" @click="suggestAnother()"
...
document.addEventListener('alpine:init', function () {
    Alpine.data('suggestApp', function () {
        var hasSuccess = {{ session('success', 0) }} > 0;
        var hasErrors = {{ $errors->any() ? 'true' : 'false' }};
        var oldType = @js(old('items.0.type', ''));
        var stored = [];
        try { stored = JSON.parse(localStorage.getItem('suggest_history') || '[]'); } catch(e) {}
        return {
            step: hasSuccess ? 4 : (hasErrors && oldType ? 3 : 1),
            language: '{{ old('language', 'en') }}',
            currentType: oldType,
            name: @js(old('submitter_name', $authUser?->name ?? '')),
            title: '',
            reference: '',
            author: '',
            detail: '',
            ...
            get currentTypeLabel() {
                var self = this;
                var found = this.types.find(function (t) { return t.value === self.currentType; });
