...
<div class="px-4 pt-4 space-y-4">
    ...
    <a href="{{ route('member.day', $viewTodayTarget) }}{{ $dayToken }}"
       class="group relative block overflow-hidden rounded-3xl shadow-[0_8px_30px_-4px_rgba(10,98,134,0.45)] dark:shadow-[0_8px_30px_-4px_rgba(226,202,24,0.35)] hover:shadow-[0_12px_40px_-4px_rgba(10,98,134,0.55)] dark:hover:shadow-[0_12px_40px_-4px_rgba(226,202,24,0.45)] transition-all duration-300 active:scale-[0.98] ring-1 ring-accent/30">
        ...
        <div class="absolute inset-0 bg-gradient-to-br from-[#0d7eab] via-accent to-[#064a66] dark:from-[#f5e44a] dark:via-accent dark:to-[#b8960b]"></div>
        ...
        <div class="relative flex items-center gap-4 p-5 sm:p-6">
            {{-- Day number badge --}}
            @if($season && $today)
            <div class="shrink-0 w-16 h-16 sm:w-[4.5rem] sm:h-[4.5rem] rounded-2xl bg-white/20 backdrop-blur-md border border-white/30 flex flex-col items-center justify-center shadow-[inset_0_1px_0_rgba(255,255,255,0.3),0_4px_12px_rgba(0,0,0,0.15)]">
                <span class="text-3xl sm:text-4xl font-black text-white dark:text-gray-900 leading-none drop-shadow-sm">{{ $today->day_number }}</span>
                <span class="text-[9px] sm:text-[10px] font-bold text-white/80 dark:text-gray-900/70 uppercase tracking-wider">{{ __('app.of_total', ['total' => $season->total_days]) }}</span>
            </div>
            @endif

            {{-- Text content --}}
            <div class="flex-1 min-w-0">
                <p class="text-xs sm:text-sm font-semibold text-white/80 dark:text-gray-900/70 mb-0.5">
                    {{ now()->locale('en')->translatedFormat('l, j F Y') }}
                </p>
                <h2 class="text-xl sm:text-2xl font-black text-white dark:text-gray-900 leading-tight drop-shadow-sm">
                    {{ $today ? __('app.view_today') : __('app.view_recommended_day') }}
                </h2>
                @if($today && $today->weeklyTheme)
                <p class="text-sm sm:text-base font-medium text-white/75 dark:text-gray-900/60 mt-0.5 truncate">
