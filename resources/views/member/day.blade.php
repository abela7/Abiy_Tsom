...
<div x-data="dayPage()" class="px-4 pt-4 space-y-4">
    ...
    <div data-tour="day-header" class="rounded-2xl bg-card border border-border shadow-sm overflow-hidden">
        ...
        <a href="{{ route('member.week', $daily->weeklyTheme) }}" class="flex items-center gap-4 px-4 py-4 bg-accent/5 hover:bg-accent/10 active:scale-[0.98] transition-all group">
            <div class="shrink-0 w-10 h-10 rounded-xl bg-accent/15 flex items-center justify-center">
                <i class="bi bi-calendar-week text-accent text-base"></i>
            </div>
            <div class="flex-1 min-w-0">
                <span class="block text-base font-bold text-accent">{{ __('app.week', ['number' => $daily->weeklyTheme->week_number]) }} &mdash; {{ localized($daily->weeklyTheme, 'name') ?? $daily->weeklyTheme->name_en ?? $daily->weeklyTheme->name_geez ?? '-' }}</span>
                <span class="block text-xs text-muted-text mt-0.5">{{ __('app.week_tap_to_read') }}</span>
            </div>
            <svg class="w-6 h-6 text-accent group-hover:translate-x-0.5 transition-all shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
        </a>
        @endif
    ...
    <div class="bg-card rounded-2xl shadow-sm border border-border overflow-hidden"
         x-data="{
            ...
            pickTheme(t){ this.readerTheme=t; localStorage.setItem('lecReaderTheme',t); this.activeShelf=null; this.lockShelfTap(); this.$nextTick(()=>this._dbgTheme(t)); },
            ...
            _dbgTheme(t){ const el=document.querySelector('[data-lec-fs]'); if(!el){console.warn('FS not found');return;} const cs=getComputedStyle(el); const g=k=>cs.getPropertyValue(k).trim(); console.log('--- LecTheme: '+t+' ---'); console.log('el classes: '+el.className.split(' ').filter(c=>c==='dark'||c==='theme-sepia').join(',')); console.log('html classes: '+document.documentElement.className); console.log('--app-surface='+g('--app-surface')+' --color-surface='+g('--color-surface')); console.log('--app-card='+g('--app-card')+' --color-card='+g('--color-card')); console.log('--app-primary='+g('--app-primary')+' --color-primary='+g('--color-primary')); console.log('--app-accent='+g('--app-accent')+' --color-accent='+g('--color-accent')); console.log('bg='+cs.backgroundColor+' color='+cs.color); }
         }"
         @keydown.escape.window="if(fullscreen) closeFullscreen()">
        ...
        <template x-if="fullscreen">
            <div data-lec-fs class="fixed inset-0 z-[100] flex flex-col bg-surface"
