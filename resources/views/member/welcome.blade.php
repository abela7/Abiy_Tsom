...
<div x-data="onboarding()"
    ...
    <div x-show="!hasToken" x-transition>
        <div class="bg-card rounded-2xl sm:rounded-3xl shadow-xl shadow-black/5 dark:shadow-black/20 border border-border overflow-hidden">

            {{-- Progress bar --}}
            <div class="h-1.5 bg-muted">
                <div class="h-full bg-accent rounded-r-full transition-all duration-500 ease-out"
            ...
            <div class="p-6 sm:p-8">
                ...
                <div x-show="step === 1" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-x-4" x-transition:enter-end="opacity-100 translate-x-0">
                    <div class="text-center mb-6">
                        <h2 class="text-xl font-bold text-primary">{{ __('app.onboarding_title') }}</h2>
                        <p class="text-sm text-muted-text mt-2">{{ __('app.onboarding_subtitle') }}</p>
                    </div>
                ...
                <div x-show="step === 2" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-x-4" x-transition:enter-end="opacity-100 translate-x-0">
                    <div class="text-center mb-6">
                        ...
                        <h2 class="text-xl font-bold text-primary">{{ __('app.wizard_whatsapp_title') }}</h2>
                        <p class="text-sm text-muted-text mt-2 leading-relaxed max-w-xs mx-auto">{{ __('app.wizard_whatsapp_description') }}</p>
                    </div>
                    ...
                    <button @click="step = 1" class="w-full mt-4 py-2 text-sm text-muted-text hover:text-secondary transition">
                        {{ __('app.wizard_back') }}
                    </button>
                </div>
                ...
                <div x-show="step === 3" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-x-4" x-transition:enter-end="opacity-100 translate-x-0">
                    <div class="text-center mb-6">
                        <div class="inline-flex items-center justify-center w-12 h-12 rounded-xl bg-accent/10 mb-3">
                            <svg class="w-6 h-6 text-accent" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/>
                            </svg>
                        </div>
                        <h2 class="text-xl font-bold text-primary">{{ __('app.wizard_phone_title') }}</h2>
                        <p class="text-sm text-muted-text mt-1.5">{{ __('app.wizard_phone_subtitle') }}</p>
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        <input type="tel"
                               ...
                               class="w-full px-4 py-3.5 border rounded-xl bg-muted/50 dark:bg-muted/30 text-primary text-lg placeholder:text-muted-text focus:ring-2 focus:ring-accent outline-none transition font-mono tracking-widest text-center"
                               :class="phone && !isPhoneValid ? 'border-red-400 focus:ring-red-400 focus:border-red-400' : 'border-border focus:border-accent'"
                               dir="ltr">
                        <p x-show="phone && !isPhoneValid" x-transition class="text-xs text-red-500 mt-2 text-center">{{ __('app.wizard_phone_invalid') }}</p>
                    </div>
                    ...
                    <button @click="step = 2" class="w-full mt-4 py-2 text-sm text-muted-text hover:text-secondary transition">
                        {{ __('app.wizard_back') }}
                    </button>
                </div>
                ...
                <div x-show="step === 4" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-x-4" x-transition:enter-end="opacity-100 translate-x-0">
                    <div class="text-center mb-6">
                        ...
                        <h2 class="text-xl font-bold text-primary">{{ __('app.wizard_time_title') }}</h2>
                        <p class="text-sm text-muted-text mt-1.5">{{ __('app.wizard_time_subtitle') }}</p>
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        <input type="time"
                               ...
                               class="w-full px-4 py-3.5 border border-border rounded-xl bg-muted/50 dark:bg-muted/30 text-primary text-lg focus:ring-2 focus:ring-accent focus:border-accent outline-none transition text-center">
                        <p class="text-xs text-muted-text mt-2 text-center">{{ __('app.wizard_time_help') }}</p>
                    </div>
                    ...
                    <button @click="step = 3" :disabled="isLoading" class="w-full mt-4 py-2 text-sm text-muted-text hover:text-secondary transition disabled:opacity-50">
                        {{ __('app.wizard_back') }}
                    </button>
                </div>

                {{-- Error message --}}
                <div x-show="errorMessage" x-transition class="mt-5 p-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl text-sm text-red-700 dark:text-red-300 text-center">
