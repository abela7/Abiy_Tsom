                ...
                <div class="border border-border rounded-2xl overflow-hidden">
                        ...
                        <div>
                            ...
                            @if($editing?->image_en)
                                <div class="mb-2 flex items-end gap-3">
                                    <img src="{{ Storage::disk('public')->url($editing->image_en) }}" alt="" class="h-24 rounded-xl object-cover">
                                    <label class="inline-flex items-center gap-2 text-sm text-red-500 hover:text-red-600 cursor-pointer">
                                        <input type="checkbox" name="remove_image_en" value="1" class="rounded border-border text-red-500 focus:ring-red-400">
                                        {{ __('app.remove') }}
                                    </label>
                                </div>
                            @endif
                            ...
                            <p class="mt-1 text-xs text-muted-text">Shown when the app is in English. Falls back to the default image if not set.</p>
                            @error('image_en')
                                <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div x-show="lang === 'am'" x-cloak class="p-4 space-y-4 bg-card">
                        ...
                        <div>
                            ...
                            @if($editing?->image)
                                <div class="mb-2 flex items-end gap-3">
                                    <img src="{{ Storage::disk('public')->url($editing->image) }}" alt="" class="h-24 rounded-xl object-cover">
                                    <label class="inline-flex items-center gap-2 text-sm text-red-500 hover:text-red-600 cursor-pointer">
                            ...
                            <input type="file" name="image" accept="image/*"
                                   class="w-full text-sm text-muted-text file:mr-3 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-sm file:font-semibold file:bg-accent/10 file:text-accent hover:file:bg-accent/20 transition">
                            <p class="mt-1 text-xs text-muted-text">{{ __('app.fundraising_am_fallback_note') }}</p>
                            @error('image')
                                <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
        ...
        <div class="space-y-3">
            ...
            <div class="p-4 rounded-xl border border-border bg-surface space-y-3">
                ...
                <div class="flex items-start gap-3">
                    @if($banner->image)
                        <img src="{{ Storage::disk('public')->url($banner->image) }}" alt="" class="w-16 h-12 rounded-lg object-cover shrink-0">
                    @endif
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center gap-2 mb-0.5 flex-wrap">
                            <p class="font-medium text-primary text-sm truncate">{{ $banner->title }}</p>
                            @if($banner->is_active)
                                <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400 whitespace-nowrap">
                                    <span class="w-1.5 h-1.5 rounded-full bg-green-500"></span>
                                    {{ __('app.active') }}
                                </span>
                            @else
                                <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-muted text-muted-text whitespace-nowrap">
                                    {{ __('app.inactive') }}
                                </span>
                            @endif
                        </div>
                        @if($banner->description)
                            <p class="text-xs text-muted-text line-clamp-1">{{ $banner->description }}</p>
                        @endif
                ...
                <div class="flex items-center gap-1 border-t border-border pt-2">
                    <form method="POST" action="/admin/banners/{{ $banner->id }}/toggle" class="inline">
                        @csrf
                        <button type="submit" class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium hover:bg-muted transition {{ $banner->is_active ? 'text-green-600 dark:text-green-400' : 'text-muted-text' }}">
                            @if($banner->is_active)
                    ...
                    <form method="POST" action="/admin/banners/{{ $banner->id }}"
                        ...
                        @method('DELETE')
                        <button type="submit"
                                class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 transition">
